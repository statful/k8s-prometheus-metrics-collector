...
KUBE_OPTS="-Dkubernetes.api.host=${KUBE_API_HOST:-kubernetes.default.svc} -Dkubernetes.api.port=${KUBE_API_PORT:-443} -Dkubernetes.api.cert=${KUBE_API_CERT:-/var/run/secrets/kubernetes.io/serviceaccount/ca.crt -Dkubernetes.api.token=${KUBE_API_CERT:-/var/run/secrets/kubernetes.io/serviceaccount/token}"
